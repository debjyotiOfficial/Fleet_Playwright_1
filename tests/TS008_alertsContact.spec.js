...
test.describe('Alert Contact', () => {
  ...
  test('should add email and phone contacts and configure alert settings', async ({ page }) => {
    ...
    // Click on submit button in email tab
    const emailSubmitButton = page.locator('#email_tab').getByRole('button', { name: 'Submit' });
    await emailSubmitButton.waitFor({ state: 'visible' });
    
    // Ensure the Alerts Contacts section is in view first
    await page.locator('heading:has-text("Alerts Contacts")').scrollIntoViewIfNeeded();
    await emailSubmitButton.scrollIntoViewIfNeeded();
    
    await emailSubmitButton.click({ force: true });
    ...
    // Click on email tab
    await page.locator(config.selectors.alertsContact.emailTab)
      .click();
