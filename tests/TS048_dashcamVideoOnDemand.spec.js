...
test.describe('Dashcam Video On Demand', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    test('should add/edit a device', async ({ page }) => {
        ...
        await page.locator(config.selectors.dashcam.videoOnDemandTable).scrollIntoView();
        ...
        const row = page.locator('#video-on-request-table tbody tr').filter({ hasText: '05/21/2025' });
        await row.locator('button.viewReportBtn').click();

        await page.waitForTimeout(5000);

        // Now click the first "Request Download" button in the file listing table
        const requestDownloadBtn = page.locator('#video-on-request-file-listing button.request-btn').filter({ hasText: 'Request Download' });
        await requestDownloadBtn.click();

        // Now click the first "View" button in the file listing table
        const viewBtn = page.locator('#video-on-request-file-listing button.view-btn').filter({ hasText: 'View' });
        await viewBtn.click();

        // Verify that 05/21/2025 is present under Start Date column and click its Delete button
        const deleteRow = page.locator('#video-on-request-table tbody tr').filter({ hasText: '05/21/2025' });
